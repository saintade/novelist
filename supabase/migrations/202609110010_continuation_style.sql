create function public.save_continuation_style(target_book text, reference_source uuid, expected_revision integer, expected_profile uuid, style_instructions text, style_metadata jsonb) returns uuid
...
begin
  ...
  select book.novel_id, book.title into novel_key, novel_title from public.books book
    ...
    where book.owner_id = auth.uid() and book.id = target_book and settings.revision = expected_revision
      and settings.reference_source_id = reference_source;
  ...
  if not exists (select 1 from public.novel_sources where owner_id = auth.uid() and id = reference_source and novel_id = novel_key and role <> 'metadata')
    or coalesce(length(trim(style_instructions)),0) not between 1 and 6000
    or jsonb_typeof(style_metadata) is distinct from 'object' then raise exception 'Invalid continuation guide'; end if;
...
$$;
revoke all on function public.save_continuation_style(text,uuid,integer,uuid,text,jsonb) from public, anon;
grant execute on function public.save_continuation_style(text,uuid,integer,uuid,text,jsonb) to authenticated;